...
$('#nav-start').blur(function() {
  routing.text2coord('start', $('#nav-start').val(), true);
});

$('#nav-end').blur(function() {
  routing.text2coord('end', $('#nav-end').val(), true);
});

$('#nav-start').bind('keypress', function(e) {
  if(e.keyCode === 13) routing.text2coord('start', $('#nav-start').val(), true);
});
$('#nav-end').bind('keypress', function(e) {
  if(e.keyCode === 13) routing.text2coord('end', $('#nav-end').val(), true);
});
...
routing.text2coord = function(type, text, center) {
  $.ajax({
    type: "GET",
    dataType: 'text',
    crossDomain: true,
    ...
    success: function(output) {
      $(output).find('place').each(function() {
        ...
        if (type === "start")
          routing.setStartPoint(obj.coord);
        if (type === "end")
          routing.setEndPoint(obj.coord);
        if (center !== undefined)
          map.panTo(obj.coord);
        return false;
      });
...
routing.road = function(data) {
  ...
  if(data.status == 0) {
    ...
    $('#nav-results').html('Brak trasy między punktami');
  }
...
routing.instructions = function(data) {
  ...
  for (var i in data) {
    ...
    if (item[5] !== '') {
      var dist = item[5].substring(0,item[5].length-1);
      dist = Math.round(parseFloat(dist/1000.0) * 100) / 100;
      text += '<td class="distance">' + dist +'&nbsp;km</td>';
    }
    text += '</tr>';
  }
  ;
