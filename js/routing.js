...
routing.setStartPoint = function(point) {
    if(routing.startPoint !== undefined)
        routing.points.removeLayer(routing.startPoint);
    routing.startPoint = L.marker(point, {draggable:true})
        .on('dragend', function() {
            routing.script("road");
        })
        .on('drag', function() {
...
routing.setEndPoint = function(point) {
    if(routing.endPoint !== undefined)
        routing.points.removeLayer(routing.endPoint);
    
    routing.endPoint = L.marker(point, {draggable:true})
        .on('dragend', function() {
            routing.script("road");
        })
        .on('drag', function() {
